...
def clean_aitimes_content(text):
    text = text.replace('"', "'")
    text = re.sub(r'^[\(\[]\s*AI타임스\s*=\s*[^)\]]*기자[\)\]]\s*', ' ', text)
    text = re.sub(r'[\(\[]\s*(?:사진|영상|이미지)\s*=\s*.*?[\)\]]\s*[.,]?', ' ', text)
    text = re.sub(r'[\(\[]?\s*사진\s*=.*?[\)\]]', ' ', text)
    text = re.sub(r'[\(\[]?\s*영상\s*=.*?[\)\]]', ' ', text)
    text = re.sub(r'[\(\[]?.*?유튜브\s*채널.*?[\)\]]', ' ', text)
    text = re.sub(r'AI타임스\s*\[관련기사\].*$', ' ', text)
...
class AITimesExtractor(BaseExtractor):
    ...
    def extract(self, soup):
        ...
        if content_div:
            ...
            content = clean_aitimes_content(content)
